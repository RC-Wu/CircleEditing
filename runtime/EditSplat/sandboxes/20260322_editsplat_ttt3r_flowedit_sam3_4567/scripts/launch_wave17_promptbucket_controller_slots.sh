...
if [[ -e "$WAVE_ROOT" ]]; then
  echo "[wave17-controller] wave root already exists: $WAVE_ROOT" >&2
  exit 2
fi

mkdir -p "$LOG_DIR"
echo "[wave17-controller] start $(date -u +%Y-%m-%dT%H:%M:%SZ)"
echo "[wave17-controller] root=$ROOT"
echo "[wave17-controller] wave_root=$WAVE_ROOT"
...
for gpu in 4 5 6 7; do
  ...
  echo "[wave17-controller] launch gpu=$gpu script=$script_path log=$log_path"
...
echo "[wave17-controller] done $(date -u +%Y-%m-%dT%H:%M:%SZ) rc=$rc"
exit "$rc"
