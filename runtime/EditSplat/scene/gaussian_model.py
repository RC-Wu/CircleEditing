...
def _fallback_nn_dist2(points_cuda: torch.Tensor, chunk: int = 2048) -> torch.Tensor:
    """Nearest-neighbor squared distance fallback when distCUDA2 fails."""
    pts_np = points_cuda.detach().float().cpu().numpy()

    # Fast fallback on CPU if scipy is available.
    try:
        from scipy.spatial import cKDTree  # type: ignore

        dists, _ = cKDTree(pts_np).query(pts_np, k=2, workers=-1)
        nn = np.asarray(dists[:, 1], dtype=np.float32)
        return torch.from_numpy(nn * nn).to(points_cuda.device)
    except Exception:
        pass

    # Dependency-free fallback: chunked pairwise distances.
    pts = points_cuda.float()
    n = pts.shape[0]
    out = torch.full((n,), float("inf"), device=pts.device, dtype=torch.float32)
    for i in range(0, n, chunk):
        j = min(i + chunk, n)
        q = pts[i:j]
        d2 = torch.cdist(q, pts).pow_(2)
        diag = torch.arange(j - i, device=pts.device)
        d2[diag, i + diag] = float("inf")
        out[i:j] = d2.min(dim=1).values
    return out
...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        pts_cuda = torch.from_numpy(np.asarray(pcd.points)).float().cuda()
        try:
            dist2 = torch.clamp_min(distCUDA2(pts_cuda), 0.0000001)
        except Exception as e:
            print(f"[WARN] distCUDA2 failed ({type(e).__name__}: {e}); using KDTree/cdist fallback.")
            dist2 = torch.clamp_min(_fallback_nn_dist2(pts_cuda), 0.0000001)
